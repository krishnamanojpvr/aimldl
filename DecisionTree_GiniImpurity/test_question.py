...
class DecisionTreeClassifierGini:
    ...
    def gini_impurity(self, y):
        ...
        p = counts/len(y)
        gini_impurity_val = 1-np.sum(p**2)
    ...
    def calculate_gini_split(self, y, left_indices, right_indices):
        ...
        gl = self.gini_impurity(y[left_indices])
        gr = self.gini_impurity(y[right_indices])
        gini = (nl/n)*gl + (nr/n)*gr
        return gini
    ...
    def find_best_split(self, X, y):
        ...
        for i in range(X.shape[1]):
            ...
            thr = np.unique(xc)
            for t in thr:
                li,ri = self.split(xc,t)
                g = self.calculate_gini_split(y,li,ri)
                if g<bg:
                    bg = g
                    bf = i
                    bt = t
